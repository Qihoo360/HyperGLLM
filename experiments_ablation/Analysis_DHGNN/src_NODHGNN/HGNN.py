...
class HGNN(nn.Module):
    def __init__(self, in_ch, hypergraph_dim, hypergraph_layernum, hypergraph_drop, hidden_size, C_list, topK):
        ...
        self.weigths = nn.Linear(self.hypergraph_dim, self.hypergraph_dim)
        self.adapter = nn.Linear(self.hypergraph_dim, self.hidden_size)
        if self.topK!=-1:
            self.poolT = nn.AdaptiveAvgPool1d(self.topK)
...
#         return x # shape: (K, D)
        ...
        x = F.relu(self.weigths(x))
        x = self.adapter(x)
        if self.topK==-1:
            return x
        else:
            x = self.poolT(x.unsqueeze(0).permute(0,2,1)).permute(0,2,1)
            return x.squeeze(0) # shape: (K, D)
# # ####check
